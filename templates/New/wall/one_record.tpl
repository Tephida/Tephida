[record]
<div class="wallrecord wall_upage2" id="wall_record_{rec-id}" style="padding-bottom:4px;margin-top:5px">
    <div style="float:left;width:60px">
        <div class="ava_mini [privacy-comment][if-comments]wall_ava_mini[/if-comments][/privacy-comment]"
             id="ava_rec_{rec-id}"><a href="/u{user-id}" onClick="Page.Go(this.href); return false"><img src="{ava}"
                                                                                                         alt=""
                                                                                                         title=""/></a>{online}
        </div>
    </div>
    <div style="float:left;width:710px;margin-left:0px">
        <div class="wallauthor fl_l"><a href="/u{user-id}" onClick="Page.Go(this.href); return false">{name}</a> <span
                    class="color777">{type}</span></div>
        [owner]
        ...
        <div class="infowalltext_f clear">
            <div class="fl_l"><a href="/wall{author-id}_{rec-id}" onClick="Page.Go(this.href); return false"
            ...
            <div class="public_likes_user_block no_display" id="public_likes_user_block{rec-id}"
                 ...
                 onMouseOut="groups.wall_like_users_five_hide('{rec-id}')" style="margin-left:585px">
	...
	<textarea class="wall_inpst fast_form_width wall_fast_text" style="height:33px;color:#000;margin:0px;width:688px"
    ...
    ></textarea>
                    <div class="button_div fl_l margin_top_5">
                        <button onClick="wall.fast_send('{rec-id}', '{author-id}', 2); return false"
...
[/record]
[all-comm]
<div class="cursor_pointer" onClick="wall.all_comments('{rec-id}', '{author-id}', 1); return false"
...
[comment]
<div class="wall_fast_block" id="wall_fast_comment_{comm-id}"
    ...
    <div class="wall_fast_date fl_l">{date} [not-owner]&nbsp;-&nbsp; <a href="#"
                                                                        onClick="wall.Answer('{rec-id}', '{comm-id}', '{name}'); return false"
                                                                        id="answer_lnk">Ответить</a>[/not-owner]
    </div>
...
<div class="wall_fast_opened_form" id="fast_form">
  ...
  ></textarea>
        <div class="button_div fl_l margin_top_5">
            <button onClick="wall.fast_send('{rec-id}', '{author-id}', 2); return false" id="fast_buts_{rec-id}">
                Отправить
            </button>
        </div>
