<div class="public_obefeed" id="f{user-id}" onMouseOver="ge('public_minilink{user-id}').style.opacity = '1'"
    ...
    <div class="fl_r public_minilink" id="public_minilink{user-id}"><a href="/"
                                                                       onClick="messages.new_('{user-id}'); return false">Сообщение</a>[admin]
    ...
    <div id="close_editf{user-id}"><span id="okoffice{user-id}">{office}</span>
        <div class="color777"><span id="okphone{user-id}">{phone}</span><span id="okemail{user-id}">{email}</span></div>
    </div>
    [admin]
