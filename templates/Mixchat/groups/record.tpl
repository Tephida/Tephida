[record]<div class="wallrecord wall_upage public_wall page_bg border_radius_5 margin_top_10" id="wall_record_{rec-id}" style="padding:20px;border:0px">
...
{* <div class="wall_tell_num wall_tell_num_group fl_r">{tell_num}</div>*}
 ...
 <div class="size10 infowalltext_f clear">
  ...
  <div class="fl_r public_wall_like cursor_pointer" onClick="{like-js-function}" onMouseOver="groups.wall_like_users_five('{rec-id}')" onMouseOut="groups.wall_like_users_five_hide('{rec-id}')" id="wall_like_link{rec-id}">
   ...
   <div style="margin-top:-3px;font-size:15px" class="fl_r"><b id="wall_like_cnt{rec-id}" class="{yes-like-color}">{likes}</b></div>
  </div>
...
<div class="clear"></div>
[/record]
[all-comm]<div class="cursor_pointer" onClick="groups.wall_all_comments('{rec-id}', '{public-id}'); return false" id="wall_all_but_link_{rec-id}"><div class="public_wall_all_comm border_radius_5" id="wall_all_comm_but_{rec-id}">Показать {gram-record-all-comm}</div></div>[/all-comm]
[comment]<div class="wall_fast_block page_bg border_radius_5 margin_top_10" style="padding:10px;border:0px" id="wall_fast_comment_{comm-id}" onMouseOver="ge('fast_del_{comm-id}').style.display = 'block'" onMouseOut="ge('fast_del_{comm-id}').style.display = 'none'">
...
[comment-form]<div class="wall_fast_opened_form border_radius_5" id="fast_form">
 <input type="text" class="wall_inpst fast_form_width fast_form_width2 wall_fast_input" value="Комментировать..." id="fast_inpt_{rec-id}" onMouseDown="wall.fast_open_textarea('{rec-id}', 2); return false" style="margin:0px" />
 <div class="no_display wall_fast_texatrea" id="fast_textarea_{rec-id}">
  <textarea class="wall_inpst fast_form_width fast_form_width2 wall_fast_text" style="height:33px;color:#000;margin-top:0px" id="fast_text_{rec-id}"
	onKeyPress="if(event.keyCode == 10 || (event.ctrlKey && event.keyCode == 13)) groups.wall_send_comm('{rec-id}', '{user-id}')"
  ></textarea>
  <div class="button_div fl_l margin_top_5"><button onClick="groups.wall_send_comm('{rec-id}', '{user-id}'); return false" id="fast_buts_{rec-id}">Отправить</button></div>
