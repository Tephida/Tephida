<style type="text/css">
#audioMP{display: none !important;}
.padcont {
    padding: 0 0 15px;
}
</style>
...
<div class="menu_item {my_music-active}" id="my_music" onclick="if(event.target.id !== 'upload_btn') audio.change_tab('my_music');">
Мои аудиозаписи
<div class="al_audio_add_but icon-plus-1 bsbb" onmouseover="showTooltip(this, {text: 'Добавить аудиозапись', shift:[2,5,0]});" id="upload_btn" onclick="audio.uploadBox()">+</div>
</div>
<div class="menu_item {public_audios-active}" {public_audios}>
Аудио сообщества
[owner]<div class="al_audio_add_but icon-plus-1 bsbb" onmouseover="showTooltip(this, {text: 'Добавить аудиозапись', shift:[2,5,0]});" id="upload_btn" onclick="audio.uploadBox('{uid}'); return false;">+</div>[/owner]
</div>
...
audio.loadAll({uid}, 0);
[friends_block]audio.LoadFriends();[/friends_block]
});
</script>