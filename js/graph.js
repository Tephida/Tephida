  ...
  for (var l = 0; l < lines.length; l++) {
    ...
    if (lines[l].f && stIdx < enIdx) {
      ...
      if (lines[l].b) {
        for (var i = stIdx; i <= enIdx; i++) {
          ...
          ctx.fillRect(cx1, y + h - cy, cx2 - cx1, ps[i].y * yfactor);
        }
      } else {
        ctx.beginPath();
        var px = x + (ps[stIdx].x - stTime) * xfactor;
        ctx.moveTo(px, y + h - yDelta * yfactor);
        for (var i = stIdx; i <= enIdx; i++) {
        var cx = x + (ps[i].x - stTime) * xfactor;
        if (smooth && (i > stIdx) && (enIdx - stIdx + 1 > 2))
          ctx.bezierCurveTo(Math.min(Math.max(px, xs1[i - stIdx - 1]), cx), Math.min(y + h, Math.max(ys1[i - stIdx - 1], 0)), Math.min(Math.max(px, xs2[i - stIdx - 1]), cx), Math.min(y + h, Math.max(ys2[i - stIdx - 1], 0)), cx, y + h - (ps[i].y + yDelta) * yfactor);
        else
          ctx.lineTo(cx, y + h - (ps[i].y + yDelta) * yfactor);
        px = cx;
        }
        ctx.lineTo(px, y + h - yDelta * yfactor);
        ctx.fill();
      }
      ctx.globalAlpha = 1;
    }
    ...
    if (lines[l].b) {
      for (var i = stIdx; i <= enIdx; i++) {
        ...
        if (i < enIdx)
        if (zoomedOut)
          ctx.lineTo(cx, y + h - (ps[i + 1].y + yDelta) * yfactor);
        else {
          ctx.stroke();
          ctx.beginPath();
          ctx.moveTo(cx, y + h - (ps[i + 1].y + yDelta) * yfactor);
        }
      }
    } else {
    ...
    if (smooth || (enIdx - stIdx < 2 && !zoomedOut)) {
      ...
      for (var i = stIdx; i <= enIdx; i++) {
        ctx.beginPath();
        ctx.arc(x + (ps[i].x - stTime) * xfactor, y + h - (ps[i].y + yDelta) * yfactor, 3.5, 0, Math.PI*2, true);
        ctx.fill();
        ctx.stroke();
      }
    }
...
function drawBars(ctx, x, y, w, h, lines, labels, barW, xfactor, yfactor, yDelta) {
  for (var l = 0; l < lines.length; l++) {
    ...
    for (i = 0; i < ps.length; i++) {
      ...
      ctx.fillStyle = lines[l].hexColor;
      ctx.fillRect(cx, Math.floor(y + h - cy), barW, Math.ceil(psy * yfactor));
    }
  }
}
  ...
  this.graphDiv.innerHTML = '<div style="float: left"><canvas style="padding-top: 20px"></canvas></div>\
    <div style="float: left">\
      ...
      <div style="position: relative; height: ' + this.viewHeight + 'px;">\
        <canvas height="' + this.viewHeight + '"></canvas>\
        <canvas height="' + (this.viewHeight - 17) + '" style="position: absolute; top: 0; left: 0;"></canvas>\
...
Graph.prototype = {
  ...
  showDots: function(ev) {
    ...
    for (var l = 0; l < g.lines.length; l++) {
      ...
      if (ni > -1) {
        ...
        ctx.beginPath();
        ctx.arc(dotX, dotY, 3, 0, Math.PI*2, true);
        ctx.fill();
    ...
    for (var l = 0; l < g.lines.length; l++) {
      if (activeBar > -1) {
        ...
        if (activeLine == l) {
          ...
          ctx.fillRect(dotX, dotY, g.barsWidth, g.viewHeight);
  ...
  showDotTT: function(lineIndex) {
    ...
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.arc(line.dot.posX, line.dot.posY, 6.5, 0, Math.PI*2, true);
    ctx.stroke();
    ctx.globalAlpha = 1;
  },
  ...
  updateLines: function() {
    ...
    ctx.clearRect(0, 0, this.viewWidth, 40);
    if (this.isNegative)
  ...
  redrawWindow: function() {
    ...
    ctxS.clearRect(0, 0, this.scaleWidth, this.viewHeight);
    ...
    for (var gridY = this.localBottom; gridY < this.localTop + this.ystep; gridY += this.ystep) {
      ...
      if (gridY < this.localTop) {
        ...
        ctx.beginPath();
        ctx.moveTo(0, cy);
        ctx.lineTo(this.viewWidth, cy);
        ctx.stroke();
      }
      ctxS.fillText(formatValue(gridY), this.scaleWidth - 6, cy);
    }

    // x scale
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(0, this.viewHeight - 25, this.viewWidth, 25);
        ...
        if (formatWidth < baseWidth) {
          ...
          if (sc.sub != -1)
            for (var tm = sTm; tm <= eTm; tm = incDate(tm, sc.sub, 1)) {
              ...
              ctx.beginPath();
              ctx.moveTo(ax, this.viewHeight - 24);
              ctx.lineTo(ax, this.viewHeight - 22);
              ctx.stroke();
            }
          for (var tm = sTm; tm <= eTm; tm = incDate(tm, sc.base, 1)) {
            ...
            ctx.beginPath();
            ctx.moveTo(ax, 9);
            ctx.lineTo(ax, this.viewHeight - 24);
            ctx.stroke();
            //ctx.strokeStyle = '#9fb1c4';
            ctx.beginPath();
            ctx.moveTo(ax, this.viewHeight - 24);
            if (sc.sub == -1)
              ctx.lineTo(ax, this.viewHeight);
            else
              ctx.lineTo(ax, this.viewHeight - 20);
            ctx.stroke();
            var textpos = ax;
            if (sc.sub == -1)
              textpos += (baseWidth / 2.0);
            if (textpos - formatWidth / 2.0 > 0 && textpos + formatWidth / 2.0 < this.viewWidth)
              ctx.fillText(replaceEntities(formatDate(tm, sc.format[j])), textpos, this.viewHeight - 19);
          }
    ...
    ctx.strokeStyle = '#e6eaf0';
    ctx.lineWidth = 1;
    ctx.strokeRect(0, this.viewHeight - 24.5, this.viewWidth - 1, 26);
    ctx.strokeStyle = '#f0f2f5';
    ctx.globalAlpha = 0.5;
    ctx.beginPath();
    ctx.moveTo(1, this.viewHeight - 25.5);
    ctx.lineTo(this.viewWidth, this.viewHeight - 25.5);
    ctx.stroke();
    ctx.globalAlpha = 1;
    ...
    ctx.beginPath();
    ctx.moveTo(0, 9);
    ctx.lineTo(0, this.viewHeight + 0.5);
    if (!isVisible(this.zoutLayout)) {
      ctx.lineTo(this.viewWidth, this.viewHeight + 0.5);
    }
    ctx.stroke();
  },
  ...
  setBarChartData: function(data, adjust) {
    ...
    if (this.params.multiple && this.menu.parentNode == this.mainLayout) {
      ...
      setStyle(menu, {position: 'absolute', right: '10px', top: '20px', zIndex: 1000});
    ...
    var maxWidth = Math.max(
      ctx.measureText(formatValue(this.maxValue + getYStep(this.maxValue, this.params.int_scale) + yUnits)).width, ctx.measureText(formatValue(getYStep(this.minAbsValue, this.params.int_scale) + yUnits)).width);
    this.scaleWidth = (this.lines.length == 0) ? 0 : (maxWidth + 30);
    this.scaleWidth = Math.floor(this.scaleWidth);
    this.vScale.style.width = this.scaleWidth + "px";
    this.vScale.style.height = this.height + "px";
    this.vScaleView.width = this.scaleWidth;
    ...
    for (var gridY = this.localBottom; gridY < this.localTop + this.ystep; gridY += this.ystep) {
      ...
      ctxS.fillText(formatValue(gridY) + yUnits, this.scaleWidth - 6, cy);
    }
    ...
    for (var i = 0; i < this.params.bar_chart.length; i++) {
      ...
      ctx.fillText(replaceEntities(text), textpos, this.viewHeight - 19);
    }
    ...
    ctx.beginPath();
    ctx.moveTo(0.5, 9);
    ctx.lineTo(0.5, this.viewHeight);
    ctx.lineTo(this.viewWidth, this.viewHeight);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.moveTo(1, this.viewHeight - 26);
    ctx.lineTo(this.viewWidth, this.viewHeight - 26);
    ctx.stroke();
  },
  destroy: function() {
    this.destroyDots();
  ...
  showBadBrowserBox: function() {
    var s = (window.devicePixelRatio >= 2 ? '_2x' : '');
    var content = getLang('stats_good_browser_box_msg') + '\
    <div id="stats_good_browsers" class="clear_fix">\
      <a href="https://www.mozilla-europe.org/" target="_blank" style="background: url(/images/firefox'+s+'.png) no-repeat 50% 17px;">Mozilla Firefox</a>\
      <a href="https://www.google.com/chrome/" target="_blank" style="background: url(/images/chrome'+s+'.png) no-repeat 50% 17px;">Google Chrome</a>\
      <a href="https://www.opera.com/" target="_blank" style="background: url(/images/opera'+s+'.png) no-repeat 50% 15px;">Opera</a>\
      <a href="https://www.apple.com/safari/" target="_blank" style="background: url(/images/safari'+s+'.png) no-repeat 50% 12px;">Safari</a>\
    </div>\
    <style>\
      #stats_good_browsers {\
        height: 136px;\
