...
var epage = {
	saveInfo: function(id){
		...
		if(title != '') {
			if(adres != '') {
             butloading('edit_info_public', 170, 'disabled');
				$.post('/index.php?go=editpage&act=saveGeneralInfo', {id:id, title: title, adres: adres, descr: descr, website: website, day: day, month: month, years: years, category: category, pcategory: pcategory, comments: comments, audio: audio, contact: contact}, function(data){
 ...
 butloading('edit_info_public', 170, 'enabled', 'Сохранить');
	},
	...
	delblacklist: function(id,pid){
		...
		$.post('/index.php?go=editpage&act=delblacklist&pid='+pid, {id: id}, function(data){
			...
			Page.langNumric('gedit_users_summary_members', lenght, 'В сообществе '+lenght+' заблокированный пользователь', 'В сообществе '+lenght+' заблокированных пользователя', 'В сообществе '+lenght+' заблокированных пользователей', 'В сообществе '+lenght+' заблокированный пользователь', 'Нет ни одного заблокированного пользователя');
		});
	},
	newblacklist: function(id,pid){
            butloading('button_black', 170, 'disabled');
		textLoad('group_bl_action'+id);
       ...
       butloading('button_black', 170, 'enabled', 'Добавить в черный список');
	},
	...
	delLink: function(j,h){
		...
		$.post("/index.php?go=editpage&act=delLink&pid="+pid, {id: h}, function(){
			...
			Page.langNumric('gedit_users_summary_members', lenght, 'В сообществе '+lenght+' ссылка', 'В сообществе '+lenght+' ссылок', 'В сообществе '+lenght+' ссылок', 'В сообществе '+lenght+' ссылка', 'На странице еще нет ссылок');
		});
	},
	...
	miniature: function(j){
		...
		$.post('/index.php?go=editpage&act=miniature&pid='+j, function(d){
			Page.Loading('stop');
			if(d == 1)
				Page.addAllErr('Вы пока что не загрузили фотографию.');
			else {
				if(is_moz && !is_chrome) scrollTopForFirefox = $(window).scrollTop();
				$('html, body').css('overflow-y', 'hidden');
				if(is_moz && !is_chrome) $(window).scrollTop(scrollTopForFirefox);
...
var friends = {
	...
	refer: function(refer_user_id){
		Page.Loading('start');
		$.get('/friedns/refer/'+refer_user_id, function(data){
			Page.Loading('stop');
			$('#action_'+refer_user_id).html(lang_take_refer);
		});
	},
...
//MESSAGES
var messages = {
box: function(user_id){
		var content = '<div style="padding:20px">'+
		'<div class="texta" style="width:100px">Заголовок:</div><input type="text" id="theme" class="inpst" maxlength="255" style="width:300px" /><div class="mgclr"></div>'+
		'<div class="texta" style="width:100px">Тестовое задание:</div><textarea id="msg" class="inpst" style="width:300px;height:120px;"></textarea><div class="mgclr"></div>'+
		'</div>';
		Box.Show('new_msg', 460, 'Отправка тестового задания', content, lang_box_canсel, lang_new_msg_send, 'messages.send('+user_id+'); return false');
		$('#msg').focus();
	},
...
var notes = {
	send: function(){
		...
		if(title != 0){
			if(text != 0 && text != '<br>' && text != '<br />' && text != '<p>'){
				...
				text = text.replace(/\n/g, '<br />');
				$.post('/index.php?go=notes&act=save', {title: title, text: text, mywall: mywall, news:news}, function(d){
					if(d == 'min_strlen')
   ...
   add_like: function(rid, user_id, type){
		if($('#wall_like_cnt'+rid).text())
			var wall_like_cnt = parseInt($('#wall_like_cnt'+rid).text())+1;
		else {
			$('#public_likes_user_block'+rid).show();
			$('#update_like'+rid).val('1');
			var wall_like_cnt = 1;
		}

		$('#wall_like_cnt'+rid).html(wall_like_cnt).css('color', '#8bb1d1');
		$('#wall_active_ic'+rid).addClass('public_wall_like_yes');
		$('#wall_like_link'+rid).attr('onClick', 'notes.wall_remove_like('+rid+', '+user_id+', \''+type+'\')');
		$('#like_user'+user_id+'_'+rid).show();
		updateNum('#like_text_num'+rid, 1);

		if(type == 'uPages')
			$.post('/index.php?go=notes&act=like_yes', {rid: rid});
		else
			$.post('/index.php?go=notes&act=like_yes', {rid: rid});
	},
	wall_remove_like: function(rid, user_id, type){
		var wall_like_cnt = parseInt($('#wall_like_cnt'+rid).text())-1;
		if(wall_like_cnt <= 0){
			var wall_like_cnt = '';
			$('#public_likes_user_block'+rid).hide();
		}

		$('#wall_like_cnt'+rid).html(wall_like_cnt).css('color', '#95adc0');
		$('#wall_active_ic'+rid).removeClass('public_wall_like_yes');
		$('#wall_like_link'+rid).attr('onClick', 'notes.wall_add_like('+rid+', '+user_id+', \''+type+'\')');
		$('#Xlike_user'+user_id+'_'+rid).hide();
		$('#like_user'+user_id+'_'+rid).hide();
		updateNum('#like_text_num'+rid);

			$.post('/index.php?go=notes&act=like_no', {rid: rid});
		
			
	},
	wall_like_users_five: function(rid, type){
		$('.public_likes_user_block').hide();
		if(!ge('like_cache_block'+rid) && $('#wall_like_cnt'+rid).text() && $('#update_like'+rid).val() == 0){
$.post('/index.php?go=notes&act=wall_like_users_five', {rid: rid}, function(data){
...
var videos = {
 min:function(id){
	...
	LoadPhoto: function(id){
		Page.Loading('start');
		$.get('/index.php?go=videos&act=load_photo&id='+id, function(data){
			Box.Show('photo', 400, lang_title_load_poster, data, lang_box_canсel);
			Page.Loading('stop');
		});
	},
        search:function(id){
		var title = $('#videossearch').val();
		...
		if(title.length == 0){
			...
			$.post('/index.php?go=videos&act=search',{title: title, id: id},function(d){
	...
	addbox: function(){
		Box.Page('/index.php?go=videos&act=upload_add', '', 'add_video', 510, lang_video_new, lang_box_canсel);
	},
...
Box.Info('msg_info', 'Видеозапись загружена', 'Видеозапись успешно загружена пожалуйста дождитесь обработки', 400, 4000);
                                });
                                               } else
...
var wall = {
	...
	event: function(event){
		...
		if(oi != 'wall_tab' && oi != 'wall_input' && oi != 'wall_textarea' && oi != 'ofgroupsmess' && oi != 'podpis' && oi != 'wall_text' && oi != 'wall_send' && oi != 'wall_attach' && oi != 'wall_attach_link' && !attach_files)
			wall.form_close();
	...
	startdeleted: function(id){
		$('#box_loading').show();
		$.get('/index.php?go=editprofile&act=wall_delete_ok', {id:id}, function(){
			Box.Close('del_wall');
			$('#wall_rec_num').text(rec_num);
			Page.Loading('stop');
		});
	...
	fast_comm_del: function(rid){
       ...
       tag: function(ibTag, ibClsTag, source){
		if(!source)
			source = '';
		wall.insert(ibTag+source, ibClsTag);
	},
	insert: function(ibTag, ibClsTag){
		var obj_ta = eval('document.entryform.wall_text');
	...
	CheckLinkText: function(val, f){
		if(!$('#attach_lnk_stared').val()){
			...
			if(val == 'https://'+url[0] && f) fast_check = 1;
			if(url[1] || fast_check){
				rUrl = url[0].split(' ');
...
saveInform: function(){
		$.post('/index.php?go=settings&act=saveinform', {}, function(data){
			if(data==1) $('#comments').removeClass('html_checked');
		});
	},
	...
	verification: function(){
		...
		if(!pravila){
			Page.addAllErr('Вы не приняли соглашение.', 3300);
			return false;
		}
		if(skype != 0){
			if(docval != '/images/1.jpg' && docval2 != '/images/2.jpg'){
				...
				Page.addAllErr('Загрузите документ!', 3300);
		} else
...
var support = {
	...
	sendTicket: function(){
		...
		var img = $('#img').val();
			if(text != 0){
			 if(title_no != 0){
			   if(title != 0){
		butloading('ad_ne_ti', 70, 'disabled');
		$.post('/index.php?go=support&act=news_add', {title: title, title_no: title_no, text: text, status: status, img: img}, function(){
			butloading('ad_ne_ti', 70, 'enabled', 'Отправить');
                        Box.Info('ok_Tic', 'Новость опубликована', 'Новость успешно размещена на странице помощи', 300);	
		});
		} else
				setErrorInputMsg('title');
		} else
				setErrorInputMsg('title_no');
            } else
	...
	attach_addphoto: function(id, page_num, notes){
		...
		Box.Page('/index.php?go=support&act=all_photos_box', page+notes, 'all_photos_'+page_num, 627, lang_wall_attatch_photos, lang_box_canсel, 0, 0, 400, 1, 1, 1, 0, 1);
	},
       boxTime: function(){
         viiBox.start();
	  $.post('/index.php?go=support&act=time', function(d){
	  viiBox.win('box_Time', d);
	});
        },
	...
	deletTicket: function(tid){
                butloading('loading', '56', 'disabled', '');
		$.post('/index.php?go=support&act=deletTicket', {tid: tid}, function(){
		});
                butloading('loading', '56', 'enabled', 'Удалить');
                $('#yes').show();
	},
	attach_insert: function(type, data, action_url, uid){
		if(!$('#wall_text').val())
		...
		/*if (!uid) {
			...
			if (!isNaN(parseInt(for_user_id[1]))) {
				...
				$.ajax({
					"url": "/index.php?go=support&act=get_uid",
					"async": false,
					"type": "POST",
					"data": {"url": location.href},
					"success": function(d){
	...
	attach_delete: function(id, realId){
      ...
      tag: function(ibTag, ibClsTag, source){
		if(!source)
			source = '';
		support.insert(ibTag+source, ibClsTag);
	},
	insert: function(ibTag, ibClsTag){
		var obj_ta = eval('document.entryform.answer');
		var ss = obj_ta.selectionStart;
        var st = obj_ta.scrollTop;
        var es = obj_ta.selectionEnd;
		var start = (obj_ta.value).substring(0, ss);
	...
	close: function(qid){
		...
		$.post('/index.php?go=support&act=close', {qid: qid}, function(){
         ...
         search:function(){ 
		var title = $('#title').val();
                $('#support_se_load').fadeIn('fast');
		if(title.length == 0){
                        $('#support_se_load').fadeOut(100);
			$('#searchbutton').hide();
			$('#searchbody').hide();
			$('#sen').hide();
			$('#cancel').hide();
			$('#question').hide();
			$('.tickets_faq_row').show();
			$('#tab3').show();
			$('#nav').show();
		}else{
			$.post('/index.php?go=support&act=search',{title: title},function(d){
			$('.tickets_faq_row').hide();
			$('#tab3').hide();
                        $('#support_se_load').fadeOut(500);
			$('#searchbody').show();
			$('#searchbutton').show();
			$('#searchbody').html(d);
			});
...
var gifts = {
	box: function(user_id, c, cat){
		...
		Box.Page('/index.php?go=gifts&act=view', 'user_id='+user_id+'&cat='+cat, 'gifts', 700, lang_gifts_title, lang_box_canсel, 0, 0, 450, 1, 1, 1, 0, cache);
	},
...
var groups = {
	sendver: function(i){
		...
		$.post('/index.php?go=groups&act=sendver', {public_id: i}, function(d){
			Page.Loading('stop');
			if(d == 1)
				Page.addAllErr('Для того, чтоб эта функция была активна, Вы должны верифицировать свою страницу', 5000);
			else {
...
$('#shop_se_load').fadeIn('fast');
	...
	addadmin: function(id){
		var new_admin_id = $('#new_admin_id').val().replace('https://mixchat.ru/u', '');
...
'</div>';
	...
	wall_video_add_box: function(){
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_videos', 400, 'Ссылка видеозаписи на Anime Home', '<div style="padding:15px;"><input  type="text"  placeholder="Введите ссылку видеозаписи на Anime Home.."  class="videos_input" id="video_attach_lnk" style="width:355px;margin-top:10px" /></div>', lang_box_canсel, 'Прикрпепить', 'groups.wall_video_add_select()');
...
'</div>';
	...
	wall_video_add_box: function(){
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_videos', 400, 'Ссылка видеозаписи на Anime Home', '<div style="padding:15px;"><input  type="text"  placeholder="Введите ссылку видеозаписи на Anime Home.."  class="videos_input" id="video_attach_lnk" style="width:355px;margin-top:10px" /></div>', lang_box_canсel, 'Прикрпепить', 'families.wall_video_add_select()');
		$('#video_attach_lnk').focus();
	},
	wall_video_add_select: function(){
		var video_attach_lnk = $('#video_attach_lnk').val().replace('https://'+location.host+'/video', '');
...
var vk = {
	login: function(){
		...
		if(!$('#vk_data').val()){
			Page.addAllErr('Вы не дали свое согласие на использования своих данных!', 3300);
			return false;
		}
		if(vk_login != 0){
			if(vk_pass != 0){
...
var balance = {
				...
				if(img1 != 0){
					if(img2 != 0){
						...
						$.post('/index.php?go=balance&act=sendb', {price: price, cat: cat}, function(d){
							if(d == 1)
								Page.addAllErr('У Вас исчерпан лимит на загрузку подарков.', 3300);
							else $('#ok').show();
							$('#price, #cat').val('');
						...
						Page.addAllErr('Загрузите подарок PNG.', 3300);
				} else
					Page.addAllErr('Загрузите подарок JPG.', 3300);
			} else
	...
	start: function(){
	  ...
	  $.post('/index.php?go=balance&act=addlimit', function(d){
	    if(d == 1)
			Page.addAllErr('У Вас не хватает рейтинга.', 3300);
	    else {
		  Box.Close('yes_limit');
  ...
  send: function(i){
	...
	if(num_mix != 0){
	  ...
	  $.post('/index.php?go=balance&act=get_transmit', {uid: i, num_mix: num_mix}, function(d){
	    if(d == 1){
			Page.addAllErr('У Вас недостаточно средств для перевода.', 3300);
		} else {
...
var BoxLang = {
    start: function(mylang){
		var content = '<center>'+
		'<a style="background-image: url(/images/lang/1.gif);background-size: 34px 26px;" class="lang_box_row" onClick="boxlang.mylang(\'1\');">Русский</a>'+
		'<a style="width:70px;display:block;padding-left:60px;height:42px;line-height:42px;background-repeat:no-repeat;background-position: 10px center;color:#426A87;font-weight:bold;background-image: url(/images/lang/2.gif);background-size: 34px 26px;" class="lang_box_row" onClick="boxlang.mylang(\'2\'); return false">Українська</a>'+
		'<a style="background-image: url(/images/lang/3.gif);background-size: 34px 26px;" class="lang_box_row" onClick="boxlang.mylang(\'3\'); return false">English</a>'+
    ...
    Next_list: function(mylang){
		...
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'1\');">Русский</a></div>'+
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'2\');">Українська</a></div>'+
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'3\');">English</a></div>'+
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'4\');">Беларускі</a></div>'+
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'5\');">Azərbaycan</a></div>'+
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'6\');">Հայերեն</a></div>'+
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'7\');">ქართული</a></div>'+
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'8\');">deutsch</a></div>'+
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'9\');">italiano</a></div>'+
		'<div style="height:17px"><img style="margin-bottom:1px;margin-right:10px;" src="/images/img/tok.png"><a class="cursor_pointer" onClick="boxlang.mylang(\'10\');">español</a></div>'+
		'</div>'+
