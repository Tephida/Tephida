					...
					//Если страница ответов "стена"
						...
						if($row_info OR $row['answer_text']){
							...
							if($row_info['tell_uid']){
								if($row_info['public'])
									$rowUserTell = $db->super_query("SELECT title, photo FROM `".PREFIX."_communities` WHERE id = '{$row_info['tell_uid']}'", false);
								else
						...
						if($row_info_likes){
							...
							if($row_info_likes['tell_uid']){
								if($row_info_likes['public'])
									$rowUserTell = $db->super_query("SELECT title, photo FROM `".PREFIX."_communities` WHERE id = '{$row_info_likes['tell_uid']}'", false);
								else
...
HTML;
					...
					if($row['action_type'] == 1){
						...
						if($rec_info['tell_uid']){
							if($rec_info['public'])
								$rowUserTell = $db->super_query("SELECT title, photo FROM `".PREFIX."_communities` WHERE id = '{$rec_info['tell_uid']}'", false);
							else
...
HTML;
					...
					//Если запись со стены сообщества
						...
						if($rec_info_groups['tell_uid']){
							if($rec_info_groups['public'])
								$rowUserTell = $db->super_query("SELECT title, photo FROM `".PREFIX."_communities` WHERE id = '{$rec_info_groups['tell_uid']}'", false);
							else
