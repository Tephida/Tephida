...
if ($_GET['act'] == 'upload') {
    ...
    if ($image_size <= $max_size) {
        ...
        if (in_array(strtolower($type), $allowed_files)) {
            ...
            if ($id) $rowCheck = $db->super_query("SELECT poster FROM `games` WHERE id = '{$id}'");
            ...
            if (move_uploaded_file($image_tmp, $rImg)) {
                ...
                if ($rowCheck) {
                    ...
                    $sql_ = $db->super_query("SELECT file FROM `games_files` WHERE user_id = '{$user_info['user_id']}' AND type = 'poster' AND game_id = '0'", true);
                    ...
                    if ($sql_) {
                        ...
                        $db->query("DELETE FROM `games_files` WHERE user_id = '{$user_info['user_id']}' AND type = 'poster' AND game_id = '0'");
...
if ($_GET['act'] == 'upload_swf') {
    ...
    if ($image_size <= $max_size) {
        ...
        if (strtolower($type) == 'swf') {
            ...
            if ($id) $rowCheck = $db->super_query("SELECT flash FROM `games` WHERE id = '{$id}'");
            ...
            if (move_uploaded_file($image_tmp, $rImg)) {
                ...
                if ($rowCheck) {
                    ...
                    $sql_ = $db->super_query("SELECT file FROM `games_files` WHERE user_id = '{$user_info['user_id']}' AND type = 'swf' AND game_id = '0'", true);
                    ...
                    if ($sql_) {
                        ...
                        $db->query("DELETE FROM `games_files` WHERE user_id = '{$user_info['user_id']}' AND type = 'swf' AND game_id = '0'");
...
if ($_GET['act'] == 'upload_scrin') {
    ...
    if ($image_size <= $max_size) {
        ...
        if (in_array(strtolower($type), $allowed_files)) {
            ...
            if ($id) $rowCheck = $db->super_query("SELECT id FROM `games` WHERE id = '{$id}'");
            ...
            if ($rowCheck)
                $cnRow = $db->super_query("SELECT COUNT(*) AS cnt FROM `games_files` WHERE game_id = '{$id}' AND type = 'scrin'");
            else
                $cnRow = $db->super_query("SELECT COUNT(*) AS cnt FROM `games_files` WHERE hash = '{$app_hash}' AND type = 'scrin'");
            ...
            if ($cnRow['cnt'] <= 3) {
                ...
                if (move_uploaded_file($image_tmp, $rImg)) {
                    ...
                    if ($rowCheck) {
                        ...
                        $rowHash = $db->super_query("SELECT hash FROM `games_files` WHERE game_id = '{$id}'");

                        //Вставляем в базу
                        $db->query("INSERT INTO `games_files` SET file = '{$image_rename}{$res_type}', hash = '{$rowHash['hash']}', type = 'scrin', user_id = '{$user_info['user_id']}', game_id = '{$id}'");
                        ...
                        $sql_ = $db->super_query("SELECT file FROM `games_files` WHERE hash != '{$app_hash}' AND user_id = '{$user_info['user_id']}' AND type = 'scrin' AND game_id = '0'", true);
                        ...
                        if ($sql_) {
                            ...
                            $db->query("DELETE FROM `games_files` WHERE user_id = '{$user_info['user_id']}' AND type = 'scrin' AND hash != '{$app_hash}' AND game_id = '0'");

                        }

                        //Вставляем в базу
                        $db->query("INSERT INTO `games_files` SET file = '{$image_rename}{$res_type}', hash = '{$app_hash}', type = 'scrin', user_id = '{$user_info['user_id']}'");
...
if (isset($_POST['send'])) {
    ...
    if (isset($title) and !empty($title)) {
        ...
        if ($sql_) {
            ...
            $db->query("UPDATE `games_files` SET game_id = '{$id}' WHERE hash = '{$app_hash}' AND user_id = '{$user_info['user_id']}'");
...
if ($_GET['act'] == 'del_game') {
    ...
    $row = $db->super_query("SELECT traf FROM `games` WHERE id = '{$id}'");

    if ($row) {

        //Выводим все файлы и удаляеи их
        $sql_ = $db->super_query("SELECT file, type FROM `games_files` WHERE game_id = '{$id}'", 1);
        ...
        $db->query("DELETE FROM `games_users` WHERE game_id = '{$id}'");

        //Удаляем из ленты активности
        $db->query("DELETE FROM `games_activity` WHERE game_id = '{$id}'");
...
if (isset($_POST['edtisave'])) {
    ...
    if (isset($title) and !empty($title)) {
        ...
        $db->query("UPDATE `games` SET title = '{$title}', descr = '{$descr}', width = '{$width}', height = '{$height}' WHERE id = '{$id}'");
...
if ($_GET['act'] == 'edit') {
    ...
    $row = $db->super_query("SELECT id, title, descr, poster, width, height FROM `games` WHERE id = '{$id}'");
    ...
    if ($row) {
        ...
        //Выводим скрины
        $sql_scr = $db->super_query("SELECT file, hash FROM `games_files` WHERE game_id = '{$id}' AND type = 'scrin'", true);
...
$numRows = $db->super_query("SELECT COUNT(*) AS cnt FROM `games` {$sql_where}");

$sql_l = $db->super_query("SELECT id, poster, title, traf FROM `games` {$sql_where} ORDER by `date` DESC LIMIT {$limit_page}, {$gcount}", true);
